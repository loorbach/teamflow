...
function TeamColumn({ team, employees, teamRoleTargets }: Props) {
  ...
  const teamTotalFte = teamRoleTargets.filter(obj => obj.teamId === team.id).reduce((acc, el) => acc + parseFloat(el.targetFte), 0);
  ...
  return (
    <Card ref={setNodeRef} className="border rounded p-4 min-w-64 gap-1">
      <div className="flex justify-between items-center px-1 mb-2 text-sm">
        ...
        <span className="font-mono tracking-tighter">{currentTotalFte.toFixed(1)}/{teamTotalFte.toFixed(1)}</span>
        <Button variant="secondary" size="icon" className="size-6 hover:cursor-pointer" onClick={() => setOpen(!open)}>
          <ChevronsUpDown />
        </Button>
      </div>
      {open && (
        <ul className="mb-1 px-1 space-y-0.5 text-xs text-gray-700 font-mono">
          {teamRoleTargets
            ...
            .map(target => {
              ...
              return (
                <li key={`${target.teamId}-${target.roleId}`} className="flex justify-between tracking-tight">
